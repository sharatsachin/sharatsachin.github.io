...
void search(char pat[], char txt[], int q) {
    ...
    int t = 0;
    int h = 1;
    // calculating value of (d^m-1)%q
    for (i = 0; i < M - 1; i++)
        h = (h * d) % q;
    // calculating hashes of pattern and original window
    for (i = 0; i < M; i++) {
        p = (d * p + pat[i]) % q;
        t = (d * t + txt[i]) % q;
    }
    ...
    for (i = 0; i <= N - M; i++) {
        ...
        // computing next rolling hash
        if (i < N - M) {
            t = (d * (t + q - txt[i]*h) + txt[i + M]) % q;
        }
    }
}
```
...
| Problem Type | Solvable in p.time  | Verifiable in p.time |
|--------------|---------------------|----------------------|
| P            | Yes                 | Yes                  |
| NP           | Yes                 | Yes or No*           |
